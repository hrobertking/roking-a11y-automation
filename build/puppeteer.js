...
const blur = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.$eval(selector, node => node.blur()).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'blur',
          target: selector
        });
...
const click = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.click(selector, options).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'click',
...
const focus = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.focus(selector).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'focus',
...
const hover = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.hover(selector).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'hover',
...
const launch = async (config = {}) => {
  ...
  try {
    ...
    const axe = await new AxePuppeteer(page); // set the drivers on the harness
...
const select = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.select(selector, value).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'select',
...
const sendKeys = async action => {
  ...
  return Harness.page.waitForSelector(selector).then(() => Harness.page.type(selector, keys, options).then(async () => {
    if (waitSelector) {
      await Harness.page.waitForSelector(waitSelector, waitOptions).then(async () => {
        await analyze({
          action: 'sendKeys',
...
const snapshot = async (filename = '') => {
  ...
  if (Harness.target) {
    ...
    [content, screenshot] = await Promise.all([Harness.page.$eval(Harness.target, node => node.outerHTML), Harness.page.screenshot({ ...Screenshot,
      fullPage: false,
      clip,
      path
    })]);
  } else {
    [content, screenshot] = await Promise.all([Harness.page.content(), Harness.page.screenshot({ ...Screenshot,
      path
    })]);
  }
